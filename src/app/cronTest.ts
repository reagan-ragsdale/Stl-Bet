...
export const cronTestFile = async () => {
    ...
    for (const player of individualPlayers) {
        let result = await nbaApiController.loadNba2023PlayerStatData(player.playerId)
    ...
    for (const team of arrayOfNbaTeamIds) {
        let result = await nbaApiController.loadTeamGameStats(team, 2023)
    ...
    for (const player of allPlayerInfo) {
        ...
        var averagePlayerData = NbaService.convertPlayerStatDataToPlayerStatAverageData(playerStats)
    ...
    for (const team of arrayOfNbaTeamIds) {
        ...
        console.log("Here 2")
        var averageTeamData = NbaService.convertTeamStatDataToPlayerStatAverageData(teamStats)
        console.log("Here 3")
