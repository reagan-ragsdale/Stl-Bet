...
export class DbGameBookData {
  ...
  static allSportFilterByMAxBookSeq = Filter.createCustom<DbGameBookData, { sport: string }>(async ({sport}) => {
    SqlDatabase.LogToConsole = true
    return SqlDatabase.rawFilter((whereFragment) => {
      whereFragment.sql = 'bookSeq = (select max(b.bookSeq) from DbGameBookData b where b.sportTitle = ' + whereFragment.addParameterAndReturnSqlToken(sport) + ' and b.bookId = bookId) and sportTitle = ' + whereFragment.addParameterAndReturnSqlToken(sport) + ' and commencetime >= CURRENT_DATE()'
    })
