...
export class NflController {
    ...
    static async nflGetAllTeamStatsByTeamNamesAndSeason(teams: string[], season: number): Promise<DBNflTeamGameStats[]>{
        const taskRepo = remult.repo(DBNflTeamGameStats)
        return await taskRepo.find({where:{teamName: teams, season: season}, orderBy: {gameDate: 'desc'}})
    }
    ...
    @BackendMethod({allowed:true})
    static async nflGetAllPlayerGameStatsBySpecificPlayerNameAndSeason(playerName: string, season: number): Promise<DBNflPlayerGameStats[]>{
        const taskRepo = remult.repo(DBNflPlayerGameStats)
    ...
    @BackendMethod({allowed:true})
    static async nflGetPlayerStatTotalsByPlayerIdAndSeason(playerId: number, season:number): Promise<DBNflPlayerGameStatTotals[]>{
        const taskRepo = remult.repo(DBNflPlayerGameStatTotals)
